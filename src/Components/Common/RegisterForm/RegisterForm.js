...
const RegisterForm = ({ attributes, nonce, isBackend }) => {
    ...
    // const { data, saveData, isLoading, error, refetch } = useWPAjax("rgfrApiKey", { nonce }, true);
    const data = { clientId: "AIzaSyD7JHM5Td_bkw1ipDZlgNIJDK5LeJdd1gQ", appId: "7318392321575160"};
    ...
    const onSubmit = async (data) => {
        ...
        wp.ajax
            .post("rgfr_registration", {
                ...data,
                adminEmail,
                userEmail,
                nonce,
                role: formOptions.userRole,
            })
            // eslint-disable-next-line no-unused-vars
            .done((res) => {
                ...
                <div className="rgfr-register-form">
                    ...
                    <div className="rgfr-registerForm-container">
                        <form ref={formRef} onSubmit={(e) => handleRegisterForm(e, validation, setErrorMessage, formFields, weakPassword, errorMessage, onSubmit, nonce)} className="rgfr-register-form-wrapper">
                            <div className="rgfr-register-logo-wrapper">
                                ...
                                {formHeader.header.title.text.length > 0 || formHeader.header.subTitle.text.length > 0 ? (
                                    <div className="rgfr-register-heading">
                                        ...
                                        <p className="rgfr-register-subHeading">{formHeader.header.subTitle.text}</p>
                                    </div>
                                ) : (
                                    ""
                                )}
                            ...
                            <div className="rgfr-inputField-wrapper">
                                {formFields.fields?.map((val, i) => (
                                    <div key={i} className={`rgfr-register-input-field rgfr-width-${i + 1}`}>
                                        {formFields.showLabel && (
                                            <label htmlFor={val.type} className={`rgfr-register-input-label`}>
                                                {val.label.text} {val.required && formFields.showRequiredMark && <span className="rgfr-requiredMark">*</span>}
                                            </label>
                                        )}
                                        <div className="rgfr-inputField">
                                            <input
                                                {...(val.type === "password" ? { onChange: (e) => setPass(e.target.value) } : {})}
                                                type={inputType(val.type) === "password" ? (formFields.passVisibilityIcon && !showPass ? "text" : "password") : inputType(val.type)}
                                                name={val.type}
                                                id={val.type}
                                            ...
                                            {formFields.fieldIcons && (
                                                <>
                                                    {val.icon.type === "upload" && (
                                                        <span className="rgfr-inputField-icon">
                                                            <SVG
                                                                src={val.icon.url ? val.icon.url.url : handleIcon(val.type)}
                                                                // className="rgfr-inputField-icon"
                                                                height="auto"
                                                            />
                                                        </span>
                                                    )}
                                                    {val.icon.type === "library" && <span className="rgfr-inputField-icon" dangerouslySetInnerHTML={{ __html: val.icon.svg }}></span>}
                                                </>
                                            )}
                                            {formFields.passVisibilityIcon && val.type === "password" && <div onClick={() => setShowPass(!showPass)}>{!showPass ? <ImEye className="register-eyeIcon open" /> : <ImEyeBlocked className="register-eyeIcon close" />}</div>}
                                        </div>
                                        {/* {
                        errorMessage.errors?.[val?.type]?.type === val.type && <small className="rgfr-input-errorMessage">{errorMessage?.errors?.[val?.type]?.message}</small>
                                        ...
                                        {formFields.showPassStrengthMeter && <PasswordStrength type={val.type} value={pass} key={i} />}
                                        {formFields.showPassStrengthText && val.type === "password" && <PasswordStrengthText password={pass} attributes={attributes} />}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="rgfr-registerFormBtn-wrapper">
                                <div className="rgfr-register-loader-wrapper">
                                    <input className={`rgfr-signupBtn ${button.signup.animation}`} type="submit" value={button.signup.text} disabled={isBackend || errorMessage.loading} />
                                    {button.signup.spinner.show && (isBackend || errorMessage.loading) && <span className="rgfr-loader"></span>}
                                </div>
                                <div className="rgfr-signInBtn-wrapper">
                                    {button.signin.text.first.length > 0 && <span>{button.signin.text.first}</span>}
                                    <a href={button.signin.text.link}>{button.signin.text.second}</a>
                                </div>
                            </div>
                        </form>
                        {(socialSignUp.google.show || socialSignUp.facebook.show) && socialSignUp.separator.show ? (
                            <div className="rgfr-separator">
                                {socialSignUp.separator.type === "text" && (
                                    <div className="rgfr-separatorText">
                                        <span>{socialSignUp.separator.text}</span>
                                    </div>
                                )}
                                {socialSignUp.separator.type === "line" && <hr />}
                            </div>
                        ) : (
                            ""
                        )}
                            ...
                            <div className="rgfr-social-signUpBtn">
                                {socialSignUp.google.show ? (
                                    data?.clientId ? (
                                        <LoginSocialGoogle
                                            ...
                                            onResolve={(response) =>
                                                onSubmit({
                                                    ...
                                                    username: generateUserName(response.data.email),
                                                })
                                            }
                                        ...
                                        <div className="rgfr-socialSignup-warning">
                                            <span>Client ID required for google signup</span>
                                        </div>
                                    )
                                ) : null}
                                {socialSignUp.facebook.show ? (
                                    data?.appId ? (
                                        <LoginSocialFacebook
                                            appId={data?.appId}
                                            onResolve={(response) =>
                                                onSubmit({
                                                    ...
                                                    username: `facebook_${response.data.userID}`,
                                                    email: `${response.data.userID}@facebook.com`,
                                                })
                                            }
                                            onReject={(error) => console.error(error)}
                                        >
